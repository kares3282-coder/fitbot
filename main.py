...
челленджами и обязательной подпиской на спонсоров.
...
WEBHOOK_HOST = os.getenv("RAILWAY_PUBLIC_DOMAIN", os.getenv("RENDER_EXTERNAL_URL", "https://your-app.up.railway.app"))
...
# ═══════════════════════════════════════════════════════════════
#  НОВОЕ: ПРОВЕРКА ПОДПИСКИ НА СПОНСОРОВ
# ═══════════════════════════════════════════════════════════════


async def check_subscription(bot: Bot, tg_id: int, channel: str) -> bool:
    """Проверить подписку пользователя на канал. True, если подписан."""
    try:
        ...
        # Если канал недоступен или бот не админ — не блокируем
        return True
...
def kb_sponsors() -> InlineKeyboardMarkup:
    """Клавиатура со ссылками на каналы + кнопка проверки."""
    rows = []
    for ch in SPONSOR_CHANNELS:
        username = ch.lstrip("@")
        rows.append([InlineKeyboardButton(
...
async def cb_check_subs(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    # Если онбординг не пройден — запускаем его
    if not p.get("onboarded"):
        await cb.message.edit_text(
            "1️⃣ <b>Твой пол?</b>",
...
async def cb_main_menu(cb: CallbackQuery, state: FSMContext) -> None:
    # Проверка подписки при возврате в меню
    if SPONSOR_CHANNELS:
        not_subbed = await check_all_subscriptions(cb.bot, cb.from_user.id)
        if not_subbed:
            try:
