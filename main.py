...
тренировками, напоминаниями, достижениями, стриками, друзьями и челленджами.
...
WEBHOOK_HOST = os.getenv("RENDER_EXTERNAL_URL", "fitbot-production-ab02.up.railway.app")
...
async def get_last_meal(tg_id: int) -> dict | None:
    """Последняя запись о еде — для добавления в избранное."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
...
async def generate_weight_chart(tg_id: int, days: int = 30) -> bytes | None:
    """Сгенерировать PNG-график веса за N дней."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
...
async def send_reminders_job(bot: Bot, rtype: str) -> None:
    """Рассылка напоминаний конкретного типа. Запускается каждый час."""
    ...
    for r in reminders:
        ...
        try:
            ...
            logger.info("Напоминание %s отправлено %s", rtype, r["telegram_id"])
...
async def check_streak_bonuses(bot: Bot) -> None:
    """Раз в день проверяем стрики и начисляем бонусы."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute("SELECT * FROM streaks WHERE current_streak IN (7, 30)") as cur:
            rows = [dict(r) for r in await cur.fetchall()]
    ...
    for s in rows:
        ...
        # Защита от повторного начисления — используем достижение как флаг
...
def setup_scheduler(bot: Bot) -> None:
    ...
    # Проверка стриков — раз в день в 21:00 МСК
    scheduler.add_job(
        check_streak_bonuses, "cron", hour=21, minute=0, args=[bot], id="streak_check"
    )
...
async def cb_main_menu(cb: CallbackQuery, state: FSMContext) -> None:
    ...
    try:
        ...
        # Если сообщение с фото — нельзя edit_text, отправляем новое
...
async def cb_repeat_yesterday(cb: CallbackQuery) -> None:
    """Скопировать все приёмы пищи из вчера в сегодня."""
...
async def cb_add_last_fav(cb: CallbackQuery) -> None:
    """Добавить последнее блюдо в избранное."""
...
async def _after_meal_actions(bot: Bot, tg_id: int) -> None:
    """Действия после записи еды: стрик + достижения."""
    # Достижение "первая еда"
