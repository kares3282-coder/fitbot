...
async def mark_payment_paid(charge_id: str) -> bool:
    """Помечает платёж 'paid'. True — если обновили впервые."""
    async with aiosqlite.connect(DB_PATH) as db:
        db.row_factory = aiosqlite.Row
        async with db.execute(
...
def kb_premium(p: dict | None, price_rub: int) -> InlineKeyboardMarkup:
    ...
    # Кнопка ЮKassa — только если клиент инициализирован
    if yookassa_client is not None:
        rows.append([InlineKeyboardButton(
...
async def cb_pay_card(cb: CallbackQuery) -> None:
    ...
    try:
        ...
        payment = await yookassa_client.payments.create_payment(
            params, idempotence_key=str(uuid.uuid4())
        )
        pay_url = payment.confirmation.url
        payment_id = payment.id

        # Сохраняем запись о платеже в БД (сумма в копейках)
...
async def cb_check_yk(cb: CallbackQuery) -> None:
    ...
    if payment.status == "succeeded":
        # Пытаемся пометить платёж как оплаченный — если впервые, выдаём Premium
...
async def process_successful_payment(message: Message) -> None:
    """Обработка успешной оплаты Stars (XTR). ЮKassa идёт через вебхук."""
...
async def yookassa_webhook(request: web.Request) -> web.Response:
    """Приём вебхуков от ЮKassa. ЮKassa ждёт 200 OK за 10 сек."""
    try:
    ...
    if event == "payment.succeeded":
        ...
        # Идемпотентность: если уже обработали — выходим
        is_new = await mark_payment_paid(payment_id)
        if not is_new:
...
async def main() -> None:
    ...
    app = web.Application()
    app["bot"] = bot  # даём вебхуку доступ к боту
