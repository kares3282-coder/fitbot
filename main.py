...
def fmt_premium_info(price_rub: int) -> str:
    return (
        ...
        f"{SEP}\n"
        f"<i>Оплатить можно звёздами Telegram или с внутреннего баланса (заработай на рефералах).</i>"
    )
...
async def cmd_start_ref(message: Message, command: CommandObject, state: FSMContext) -> None:
    ...
    if inviter_id and inviter_id != u.id:
        ...
        if linked:
            logger.info("Пользователь %s привязан к пригласившему %s", u.id, inviter_id)
