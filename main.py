...
# ЮKassa provider token из @BotFather (формат: 390540012:LIVE:xxxxx)
...
if not PROVIDER_TOKEN:
    logger.warning(
        "PROVIDER_TOKEN не задан — оплата картой (ЮKassa) будет недоступна. "
        "Останется только Telegram Stars."
    )
    ...
    async with aiosqlite.connect(DB_PATH) as db:
        ...
        workouts = [
            ...
            ("female", "abs", "Велосипед",
             "3 подхода по 30 секунд. Локтем к противоположному колену.",
             8, 70, "🟡 Средний"),
            ...
            ("male", "chest_arms", "Обратные отжимания от стула",
             "3 подхода по 15 раз. Спиной к стулу.",
             10, 100, "🟢 Новичок"),
            ...
            ("male", "back_shoulders", "Тяга в наклоне с бутылками",
             "3 подхода по 12 раз. Бутылки с водой как гантели.",
             12, 110, "🟢 Новичок"),
            ("male", "back_shoulders", "Разведение рук в стороны",
             "3 подхода по 15 раз с бутылками.",
             10, 80, "🟢 Новичок"),
            ("male", "back_shoulders", "Лодочка",
             "3 подхода по 20 секунд. Лёжа на животе.",
             6, 55, "🟢 Новичок"),
            ...
            ("male", "legs", "Приседания",
             "4 подхода по 20 раз. Можно с рюкзаком.",
             15, 140, "🟢 Новичок"),
            ("male", "legs", "Выпады",
             "3 подхода по 15 раз на каждую ногу.",
             12, 120, "🟢 Новичок"),
            ("male", "legs", "Подъёмы на носки",
             "4 подхода по 25 раз. Стоя на книге.",
             8, 70, "🟢 Новичок"),
            ("male", "hiit", "Табата 4 минуты",
             "20 сек работы, 10 сек отдыха, 8 раундов.",
             10, 160, "🔴 Сложный"),
            ("male", "hiit", "Круговая тренировка",
             "5 упражнений по 45 сек, 3 круга.",
             20, 250, "🟡 Средний"),
            ("male", "hiit", "HIIT для жиросжигания",
             "30 сек работы, 30 сек отдыха, 10 раундов.",
             12, 180, "🟡 Средний"),
        ]
...
async def create_stars_payment(
    ...
    """Сохранить запись о платеже (Stars или ЮKassa)."""
    async with aiosqlite.connect(DB_PATH) as db:
...
async def get_payment_stats_by_currency() -> dict:
    """Разбить статистику по валютам: XTR (Stars) и RUB (ЮKassa)."""
...
async def update_challenge_weight(tg_id: int, challenge_id: str, weight: float) -> None:
    async with aiosqlite.connect(DB_PATH) as db:
        await db.execute(
            "UPDATE challenge_participants SET current_kg = ? "
            "WHERE telegram_id = ? AND challenge_id = ?",
            (weight, tg_id, challenge_id),
        )
        await db.commit()
...
def fmt_premium_info(price_rub: int) -> str:
    stars_line = f"⭐ <b>{SUBSCRIPTION_STARS} звёзд</b> (Telegram Stars)"
    cards_line = f"💳 <b>{price_rub} ₽</b> (карта РФ / СБП)"
    ...
    return (
        ...
        f"{SEP}\n"
        f"<i>Stars — универсально (работает на iOS). Карта/СБП — для России.</i>"
    )
...
def kb_premium(p: dict | None, price_rub: int) -> InlineKeyboardMarkup:
    """Клавиатура Premium: Stars + ЮKassa (карта/СБП) + баланс."""
    rows = [
        [InlineKeyboardButton(
    ...
    # Кнопка ЮKassa — только если provider_token задан в .env
    if PROVIDER_TOKEN:
        rows.append([InlineKeyboardButton(
...
async def cb_pay_card(cb: CallbackQuery) -> None:
    """Оплата через ЮKassa (карты РФ, СБП, Т-Банк)."""
    if not PROVIDER_TOKEN:
        await cb.answer(
    ...
    await cb.answer()
    try:
        # Сумма для ЮKassa указывается в копейках
        await cb.bot.send_invoice(
            chat_id=cb.from_user.id,
            title="Premium подписка FitBot",
            description=f"Безлимитные анализы еды и тренировки на {SUBSCRIPTION_DAYS} дней",
            payload="premium_30d_card",
            provider_token=PROVIDER_TOKEN,     # ← ЮKassa token
            currency="RUB",
            prices=[LabeledPrice(label="Premium", amount=price_rub * 100)],
            start_parameter="premium_card",
            # Чек для ЮKassa (54-ФЗ). Для самозанятых vat_code=1 (без НДС)
            provider_data=json.dumps({
                "receipt": {
                    "items": [
                        {
                            "description": "Premium подписка FitBot",
                            "quantity": 1.00,
                            "amount": {
        ...
        logger.exception("Ошибка отправки счёта ЮKassa")
        await cb.message.answer(
...
async def process_pre_checkout(query: PreCheckoutQuery) -> None:
    """Подтверждение готовности принять платёж (10 секунд на ответ)."""
    # Для тестов всегда ok=True. В продакшене — проверяй остатки, лимиты и т.п.
    await query.answer(ok=True)


@router.message(F.successful_payment)
async def process_successful_payment(message: Message) -> None:
    """Обработка успешной оплаты — Stars или ЮKassa."""
    payment = message.successful_payment
    payload = payment.invoice_payload
    currency = payment.currency

    logger.info(
        "Успешный платёж: user=%s, amount=%s %s, payload=%s, charge_id=%s",
    ...
    if payload.startswith("premium_30d"):
        ...
        if currency == "XTR":
            paid_line = f"⭐ Оплачено: <b>{payment.total_amount} звёзд</b>"
        else:
            # ЮKassa: total_amount в копейках
            rub = payment.total_amount / 100
            paid_line = f"💳 Оплачено: <b>{rub:.2f} ₽</b>"
...
async def cb_adm_stats(cb: CallbackQuery) -> None:
    ...
    rub_sum = rub["total"] / 100  # копейки → рубли
...
async def main() -> None:
    ...
    logger.info(
        "FitBot запущен. Порт %d | Провайдер оплаты: %s",
        PORT,
        "Stars + ЮKassa" if PROVIDER_TOKEN else "только Stars",
    )
